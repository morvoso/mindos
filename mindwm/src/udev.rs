    ...
    fn render_surface(&mut self, node: DrmNode, crtc: crtc::Handle, frame_target: Time<Monotonic>) {
        ...
        let reschedule = match result {
            ...
            Err(err) => {
                ...
                if inactive {
                    // Session paused (VT switch): the activation handler repaints.
                    trace!("skipping frame, device inactive");
                } else {
                    warn!("Error during rendering: {:#?}", err);
                }
                match err {
...
fn render_surface<'a>(
    ...
    let backdrop = mindbar
        .backdrop_element(renderer, output_geometry.size, scale.x, has_windows, show_wordmark)
