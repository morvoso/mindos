...
//!
//! A second is a freeze. What a desktop is actually judged on is much
...
/// output watchdog wakes the loop every 500 ms whatever else is happening,
/// and nothing that can block for long runs on it any more -- resets wait on
/// the kernel from a helper thread, monitors are probed from another -- so a
/// second of silence is already a freeze the user can see.
...
fn describe(syscall: &str, wchan: &str) -> (String, Option<i64>) {
    ...
    // Which request: an atomic commit waiting out a stuck flip is a
    // different bug from a buffer import or a vblank wait.
    if number == Some(SYS_IOCTL) {
        let request = syscall
            .split_whitespace()
            .nth(2)
            .and_then(|arg| u64::from_str_radix(arg.trim_start_matches("0x"), 16).ok());
        if let Some(request) = request {
            call = format!("{call} {}", crate::stall::ioctl_name(request));
        }
    }
...
mod tests {
    ...
    fn a_stall_says_which_syscall_it_is_in() {
        ...
        // Blocked on the card.
        let (what, fd) = describe("16 0x9 0xc03864bc", "nv_drm_atomic_commit");
        assert_eq!(what, "ioctl DRM_IOCTL_MODE_ATOMIC waiting in nv_drm_atomic_commit");
        assert_eq!(fd, Some(9));
