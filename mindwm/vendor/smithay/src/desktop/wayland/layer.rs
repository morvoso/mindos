use crate::{
    ...
    utils::{user_data::UserDataMap, CheckedMutex, CheckedMutexGuard, IsAlive, Logical, Point, Rectangle},
    wayland::{
...
///
/// Note: This function internally uses a [`CheckedMutex`] per
/// [`Output`] as exposed by its return type. Therefor
/// trying to hold on to multiple references of a [`LayerMap`]
/// of the same output using this function *will* panic, where upstream
/// Smithay's `Mutex` would deadlock (a MindOS change).
pub fn layer_map_for_output(o: &Output) -> CheckedMutexGuard<'_, LayerMap> {
    let userdata = o.user_data();
    userdata.insert_if_missing_threadsafe(|| {
        CheckedMutex::new(LayerMap {
            layers: IndexSet::new(),
            output: o.downgrade(),
    ...
    userdata.get::<CheckedMutex<LayerMap>>().unwrap().lock().unwrap()
}
