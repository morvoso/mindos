use crate::{
    utils::{
        hook::{Hook, HookId},
        CheckedMutex, CheckedMutexGuard, Serial,
    },
...
impl PrivateSurfaceData {
    pub fn new() -> CheckedMutex<PrivateSurfaceData> {
        CheckedMutex::new(PrivateSurfaceData {
            parent: None,
            children: vec![],
            public_data: SurfaceData {
    ...
    pub fn lock_user_data(surface: &WlSurface) -> CheckedMutexGuard<'_, PrivateSurfaceData> {
        let inner = &surface.data::<SurfaceUserData>().unwrap().inner;
        // MindOS: the lock is held while tree walks, `with_states` and hooks
        // run compositor code, and that code reaching the same surface again
        // used to wait on itself with every display frozen.
        if inner.is_held_by_this_thread() {
            panic!(
                "{surface:?} is already locked by this thread: code running inside a surface-tree \
                 walk or a with_states closure reached the same surface's state again (use the \
                 SurfaceData the closure was given), which would wait forever"
            );
        }
        inner.lock_unchecked()
    }
