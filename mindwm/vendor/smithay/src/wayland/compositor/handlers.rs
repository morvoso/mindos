...
use crate::utils::{
    ...
    CheckedMutex, Client, Logical, Point,
};
...
pub struct SurfaceUserData {
    // MindOS: checked, so a callback that locks its own surface again panics
    // instead of freezing the compositor.
    pub(crate) inner: CheckedMutex<PrivateSurfaceData>,
