...
impl X11Surface {
    ...
    /// Reads `_NET_WM_STATE` as the client left it. Only right before the
    /// window is mapped: once it is, the WM owns the property.
    pub(super) fn update_net_state(&self) -> Result<(), ConnectionError> {
        let conn = self.conn.upgrade().ok_or(ConnectionError::UnknownError)?;
        let atoms = match conn
            .get_property(false, self.window, self.atoms._NET_WM_STATE, AtomEnum::ATOM, 0, 1024)?
            .reply_unchecked()
        {
            Ok(Some(reply)) => reply.value32().map(|vals| vals.collect::<Vec<_>>()).unwrap_or_default(),
            Ok(None) | Err(ConnectionError::ParseError(_)) => Vec::new(),
            Err(err) => return Err(err),
        };
        let mut state = self.state.lock().unwrap();
        state.net_state = atoms
            .into_iter()
            // focus and visibility are the WM's to say, whatever was left behind
            .filter(|a| *a != self.atoms._NET_WM_STATE_FOCUSED && *a != self.atoms._NET_WM_STATE_HIDDEN)
            .collect();
        Ok(())
    }
