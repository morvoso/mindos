...
use x11rb::{
    ...
    protocol::{
        ...
        xproto::{
            ...
            FontWrapper, GcontextWrapper, GetPropertyReply, ImageFormat, NotifyDetail, NotifyMode,
...
pub trait XwmHandler {
    ...
    /// Window asks to be made the active window (a `_NET_ACTIVE_WINDOW`
    /// client message).
    ///
    /// `currently_active_window` is the window the client believes is active
    /// now, when it names one this WM manages: a client moving focus between
    /// its own windows says so here. Grant the request by giving the window
    /// keyboard focus; the default does nothing.
...
pub struct X11Wm {
    ...
    // what _NET_ACTIVE_WINDOW on the root says now
    active_window: X11Window,
...
impl X11Wm {
    ...
    /// Publishes the window with keyboard focus as `_NET_ACTIVE_WINDOW`, or
    /// no window when the focus is on anything that is not an X11 window.
    ///
    /// X11 clients read this property to learn whether they are in the
    /// foreground: Wine decides which of its windows is the foreground window
    /// from it alone, and a window it does not think is in the foreground
    /// gets no keyboard input and never has its cursor clip applied.
    pub fn set_active_window(&mut self, window: Option<&X11Surface>) -> Result<(), ConnectionError> {
        let id = window
            .filter(|w| w.xwm_id() == Some(self.id) && !w.is_override_redirect())
            .map(|w| w.window_id())
    ...
    D: 'static,
{
    ...
    match event {
        ...
        Event::MapRequest(r) => {
            if let Some(surface) = xwm.windows.iter().find(|x| x.window_id() == r.window).cloned() {
                if surface.state.lock().unwrap().mapped_onto.is_none() {
                    ...
                    // EWMH: a client that wants to start fullscreen or maximised
                    // puts that in _NET_WM_STATE before it maps. Wine does, for
                    // every window that covers a monitor.
                    surface.update_net_state()?;
        ...
        // A client moving the X focus between its own windows (it may, once
        // it has been given the focus). Only a managed window counts: the
        // root and the frames see focus events too, and publishing one of
        // those as the active window tells every client that none of its
        // windows is in the foreground. Losing the focus is the compositor's
        // to publish, through `X11Wm::set_active_window`.
        Event::FocusIn(n) => {
            if n.mode != NotifyMode::GRAB
                && n.mode != NotifyMode::UNGRAB
                && !matches!(n.detail, NotifyDetail::POINTER | NotifyDetail::POINTER_ROOT | NotifyDetail::NONE)
                && n.event != xwm.active_window
                && xwm
                    .windows
                    .iter()
                    .any(|x| x.window_id() == n.event && !x.is_override_redirect())
            {
                xwm.active_window = n.event;
                conn.change_property32(
                    PropMode::REPLACE,
                    xwm.screen.root,
