...
use crate::{
    ...
    utils::Serial,
    utils::{CheckedMutex, Clock, IsAlive, Logical, Monotonic, Point},
};
...
pub struct PointerHandle<D: SeatHandler> {
    ...
    pub(crate) inner: Arc<CheckedMutex<PointerInternal<D>>>,
...
impl<D: SeatHandler + 'static> PointerHandle<D> {
    pub(crate) fn new() -> PointerHandle<D> {
        PointerHandle {
            inner: Arc::new(CheckedMutex::new(PointerInternal::new())),
            #[cfg(feature = "wayland_frontend")]
...
impl<D: SeatHandler + 'static> PointerInternal<D> {
        ...
        F: FnOnce(&mut D, &mut PointerInnerHandle<'_, D>, &mut dyn PointerGrab<D>),
    {
        ...
        // or every later pointer event panics on it.
