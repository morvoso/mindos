...
use crate::utils::{CheckedMutex, IsAlive, Logical, Point, Serial, SerialCounter};
...
pub struct TouchHandle<D: SeatHandler> {
    // MindOS: checked, because grabs and focus callbacks run with it held.
    pub(crate) inner: Arc<CheckedMutex<TouchInternal<D>>>,
    #[cfg(feature = "wayland_frontend")]
...
impl<D: SeatHandler + 'static> TouchHandle<D> {
    ...
    {
        TouchHandle {
            inner: Arc::new(CheckedMutex::new(TouchInternal::new(default_grab))),
            #[cfg(feature = "wayland_frontend")]
...
impl<D: SeatHandler + 'static> TouchInternal<D> {
        ...
        F: FnOnce(&mut D, &mut TouchInnerHandle<'_, D>, &mut dyn TouchGrab<D>),
    {
        ...
        // MindOS: a panic in the callback must not leave the grab borrowed,
        // or every later touch event panics on it.
        let ran = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| match grab {
        ...
        if let GrabStatus::Borrowed = self.grab {
            ...
            // unless it panicked: then it ends.
            self.grab = if ran.is_ok() { grab } else { GrabStatus::None };
        }
