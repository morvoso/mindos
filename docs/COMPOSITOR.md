...
| `src/udev.rs`, `src/winit.rs` | DRM/KMS and nested backends (from anvil); `udev.rs` also keeps every display drawing: the page flip deadline, refused-frame retries, the reset helper thread, the connector probe thread and the output watchdog |
...
| `src/watchdog.rs` | The loop watch: a thread outside the event loop that notices it has stopped, says where, and aborts a deadlocked compositor (see *When every display stops at once*) |
| `src/stall.rs` | Which part of the compositor the event loop is in, and DRM requests by name, for the loop watch's report |
| `src/recover.rs` | Catches a panic in one turn of the event loop instead of ending the session; gives up after a run of them |
| `src/sched.rs` | Scheduling priority: the event loop above the desktop, the programs it starts at the ordinary one |
...
* **A slow watchdog covers the rest.** Every half second, an output whose
  ...
  reset. It is the backstop for the deadline itself failing to arm.
* **What the watchdog really asks is whether a frame was drawn.** Every theory
  ...
  stopped — no matter how healthy its timers look — and its loop is started
  again. That check needs to know nothing about *why*, which is what makes it
  the one that catches the freeze nobody has thought of yet.
* **An armed timer only counts as alive until it is due.** The watchdog used to
  ...
  second past due, for two ticks in a row, has stopped, and is started again
  like any other. Two ticks, so that a loop merely blocked for a moment — a
  modeset, a slow GPU — is left alone. Nothing is wrong with the display in
  either case, only with the bookkeeping, so drawing again is all it takes;
  a display that then does not take its frame goes through the deadline and
  the resets above.
...
* **A change is drawn now if the repaint already armed is far off.** A commit
  ...
  earlier. A retry of a refused frame keeps its backoff.
...
* **A display coming or going never takes the others with it.** Plugging a
  ...
  from a cadence that no longer exists. Adding a monitor can re-allocate the
  whole device the same way, and is handled the same way.
* **Nothing else on the event loop waits on a monitor or on the journal.**
  ...
  * The log goes to journald through a `mindwm-log` thread. A write to a
    ...
    it can.
  * A `mindwm-loop-watch` thread watches the event loop from outside: see
...
timestamp every turn; the thread wakes four times a second and reads it. A
second of silence is reported — the loop is never quiet for that long even
with nothing to do, because the output watchdog's own timer wakes it twice a
second — and the report says which part of the compositor the loop is in and
*where* it is stopped, naming the DRM request when it is one, because the
kernel will tell any thread of a process where its siblings are:
...
ERROR mindwm::watchdog: the event loop has not turned: every display is
  ...
  sock_wait_data on socket:[41283] doing="talking to XWayland"
```

or, for a commit waiting out a stuck page flip, `stopped_at=ioctl
DRM_IOCTL_MODE_ATOMIC waiting in nv_drm_atomic_commit on /dev/dri/card1`.

Blocked in a read on a socket is a different bug from blocked in an ioctl on
the card, and without that line the difference is hours. Mostly it only
reports: killing whatever the loop is waiting on would as often turn a hitch
into a crash, since a GPU reset can hold the loop for seconds and let go. A
loop parked in a futex for ten seconds does not let go — that is a lock nobody
will release, which on 2026-09-12 held both displays until the machine was
switched off — so the watcher aborts the compositor then, or after thirty
seconds stuck anywhere. The session ends and the login screen comes back
instead of a hard reboot, and the core dump names the lock. A panic in one
turn of the loop is caught instead of ending the session (`recover.rs`); only
a run of them within a second stops the compositor, with exit status 70 so
that mindos-session starts it again. `get_graphics` carries the worst one of the session
