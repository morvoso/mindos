...
in the dark palette, and nothing brighter than it reaches the screen between
the boot menu and the login screen: a void gradient (`#05070a` → `#0d1016`),
the MINDOS wordmark, the line `MINDOS / BUILT FOR PLAY` above it and a 4 px
progress line in the highlight green along the bottom. Systemd's unit names
are deliberately ignored — the boot is quiet, and anything that goes wrong is
on tty2 and in the journal. Status messages appear bottom-left; the disk
password prompt draws above the wordmark, with green bullets. Text is drawn
by Plymouth itself from the fonts named in `mindos.plymouth`;
`packages/mindos-theme/gen-assets.py` renders the other theme assets —
...
`docs/img/grub-red.png`), then Plymouth shows the dark splash with the
wordmark and the progress line (`docs/img/plymouth-dark.png`).
