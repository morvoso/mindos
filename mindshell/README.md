...
layer-shell windows on every output (a desktop, the panels, transient popups),
renders the TypeScript UI in `ui/` with WebKitGTK 6.0, and exposes the system
to that UI through `window.mindos`: the compositor (mindwm IPC socket), the
StatusNotifier tray, desktop entries, icons, audio, network, battery, power
and statistics. `docs/SHELL.md` is the contract; this file is the host's
operating notes.
...
| file | what |
|---|---|
| `src/main.rs` | arguments (`--devtools`, `--ui-dir`), logging, GTK init, signals, main loop |
...
| `src/scheme.rs` | the `mindos://shell/` URI scheme (`/app`, `/icon`, `/tray`) |
| `src/ipc.rs` | compositor IPC client (newline JSON over `$MINDWM_SOCKET`), auto-reconnect |
...
| `data/` | default `layout.json`, `shell.toml`, `mindos-shell.service`, `50-mindshell` autostart |
